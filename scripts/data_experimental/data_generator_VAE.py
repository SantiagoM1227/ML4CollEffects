...
def build_datasets(
    ...
    out["X_hist"] = np.stack(X_hist_list, axis=0)     # [Ns,15,64,64]
    out["Y_hist"] = np.stack(Y_hist_list, axis=0)
    out["aux_in"] = np.stack(aux_in_list, axis=0)     # [Ns,12]
    out["aux_out"] = np.stack(aux_out_list, axis=0)   # [Ns,12]
    out["MU_raw"] = np.stack(MU_raw_list, axis=0)     # [Ns,3]
    out["MU_feat"] = np.stack(MU_feat_list, axis=0)   # [Ns,3]
