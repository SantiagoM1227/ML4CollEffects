...
class TrackingTransformer(nn.Module):
    def __init__(self, latent_dim: int = 256, d_model: int = 512, n_layers: int = 6, n_heads: int = 8, dropout: float = 0.1):
        ...
        self.out = nn.Sequential(nn.LayerNorm(d_model), nn.Linear(d_model, latent_dim))

    @staticmethod
    def causal_mask(N: int, device) -> torch.Tensor:
        return torch.triu(torch.ones(N, N, device=device, dtype=torch.bool), diagonal=1)

    def forward_teacher_forcing(self, z_traj_in: torch.Tensor, elem_tokens: torch.Tensor) -> torch.Tensor:
        B, N, _ = elem_tokens.shape
        assert z_traj_in.shape[1] == N
        zt = self.z_proj(z_traj_in)
        x = torch.cat([zt, elem_tokens], dim=-1)
        x = self.fuse(x)
        h = self.tr(x, mask=self.causal_mask(N, x.device))
        dz = self.out(h)
        return z_traj_in + dz

    #@torch.no_grad()
    def forward_autoregressive(self, z0: torch.Tensor, elem_tokens: torch.Tensor) -> torch.Tensor:
        """
        Autograd-safe autoregressive rollout.
        ...
        for t in range(N):
            if t == 0:
                z_traj_in = z_prev[:, None, :]               # [B,1,latent] (z0)
            else:
                z_traj_in = torch.stack([z0] + z_out, dim=1) # [B,t+1,latent] (z0..z_t)
    ...
    @torch.no_grad()
    def forward_autoregressive_nograd(self, z0: torch.Tensor, elem_tokens: torch.Tensor) -> torch.Tensor:
        return self.forward_autoregressive(z0, elem_tokens)
...
class BeamLatentTrackingModel(nn.Module):
    def __init__(self, mu_dim: int, latent_dim: int = 256, d_model: int = 512):
        ...
        self.tracker = TrackingTransformer(latent_dim=latent_dim, d_model=d_model)
    ...
    def forward(
        ...
        *,
        ...
        if mode.upper() == "TF":
            ...
            if ar_nograd:
                z_pred = self.tracker.forward_autoregressive_nograd(z0, elem_tokens)
            else:
