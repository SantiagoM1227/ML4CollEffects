#!/bin/bash
#SBATCH --job-name=demo005-pipeline
#SBATCH --output=/pbs/home/s/smartinez/ML4CollEffects/outputs/demo005-pipeline-%j.out
#SBATCH --error=/pbs/home/s/smartinez/ML4CollEffects/outputs/demo005-pipeline-%j.err
#SBATCH --ntasks=1
...
export VAE_OUTDIR="$DEMO005_ROOT/output/stage1_vae"
export DYN_OUTDIR="$DEMO005_ROOT/output/stage2_dynamics"
mkdir -p "$VAE_OUTDIR" "$DYN_OUTDIR"

export VAE_CKPT="$VAE_OUTDIR/vae_best.pt"
export DYN_CKPT="$DYN_OUTDIR/dyn_best.pt"

echo "=== demo005 pipeline ==="
...
VAE_EVAL_JOBID=$(sbatch --parsable --dependency=afterok:${SLURM_JOB_ID} \
  --export=ALL,DATASET_PATH="$DATASET_PATH",DEMO005_ROOT="$DEMO005_ROOT",DEVICE="$DEVICE",VAE_CKPT="$VAE_CKPT" \
  "$DEMO005_ROOT/sbatch_eval_vae.sh")
...
DYN_EVAL_JOBID=$(sbatch --parsable --dependency=afterok:${SLURM_JOB_ID} \
  --export=ALL,DATASET_PATH="$DATASET_PATH",DEMO005_ROOT="$DEMO005_ROOT",DEVICE="$DEVICE",VAE_CKPT="$VAE_CKPT",DYN_CKPT="$DYN_CKPT" \
  "$DEMO005_ROOT/sbatch_eval_dyn.sh")
